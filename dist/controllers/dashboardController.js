...
export async function getDashboardStats(req, res) {
    try {
        ...
        const totalGuestsThisMonth = thisMonthBookings.reduce((sum, b) => sum + b.numGuests, 0);
        const totalRevenue = allBookings.reduce((sum, b) => sum + Number(b.totalPriceSar), 0);
        const totalRevenueThisMonth = thisMonthBookings.reduce((sum, b) => sum + Number(b.totalPriceSar), 0);
        ...
        for (let i = 5; i >= 0; i--) {
            ...
            const monthBookings = allBookings.filter((b) => b.createdAt >= monthStart && b.createdAt <= monthEnd);
            monthlyTrend.push({
                month: monthStart.toLocaleDateString('en-US', { month: 'short', year: 'numeric' }),
                bookings: monthBookings.length,
                guests: monthBookings.reduce((sum, b) => sum + b.numGuests, 0),
                revenue: monthBookings.reduce((sum, b) => sum + Number(b.totalPriceSar), 0),
            });
        }
        res.json({
            success: true,
