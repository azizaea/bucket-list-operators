 ...
 */
export async function generateMonthlyBookingsReport(req, res) {
    try {
 ...
 */
export async function generateRevenueReport(req, res) {
    try {
        const operatorId = req.user?.operatorId;
        if (!operatorId) {
            res.status(401).json({
 ...
 */
export async function generateDemographicsReport(req, res) {
    try {
        const operatorId = req.user?.operatorId;
        if (!operatorId) {
            res.status(401).json({
...
export async function exportReportCSV(req, res) {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="ministry-report-${reportType}-${year}-${month}.csv"`);
        res.send(csvContent);
    }
    catch (error) {
